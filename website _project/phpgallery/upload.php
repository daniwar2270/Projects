...
if (isset($_FILES['image'], $_POST['title'], $_POST['description'])) {
	...
	if (!empty($_FILES['image']['tmp_name']) && getimagesize($_FILES['image']['tmp_name'])) {
		if (file_exists($image_path)) {
			...
			$stmt = $pdo->prepare('INSERT INTO images (title, description, filepath, uploaded_date,user_id) VALUES (?, ?, ?, CURRENT_TIMESTAMP,?)');
	        $stmt->execute([ $_POST['title'], $_POST['description'], $image_path, $_SESSION['Id'] ]);
